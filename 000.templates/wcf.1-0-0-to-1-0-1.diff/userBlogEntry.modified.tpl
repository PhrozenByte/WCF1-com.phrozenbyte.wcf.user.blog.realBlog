...
<div id="main">
	...
	<div class="mainHeadline">
		...
		<div class="headlineContainer">
			<h2{if $entry->isEditable()} id="userBlogEntryTitle{@$entry->entryID}" class="blogArticleTitle"{/if}><a href="index.php?page=UserBlogEntry&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}">{$entry->subject}</a></h2>
			{if USER_BLOG_ENABLE_RATING}<p><span id="com.woltlab.wcf.user.blog.entry-ratingOutput{@$entry->entryID}">{@$entry->getRatingOutput()}</span></p>{/if}
		</div>
	</div>
