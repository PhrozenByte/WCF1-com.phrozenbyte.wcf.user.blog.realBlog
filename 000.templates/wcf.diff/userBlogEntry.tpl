				...
				<div class="container-1 column first">
					<div class="columnInner">
						<div class="contentBox blogArticle">
							...
							<h3 class="subHeadline{if $entry->isEditable()} blogArticleTitle" id="userBlogEntryTitle{@$entry->entryID}{/if}"><a href="index.php?page=UserBlogEntry&amp;entryID={@$entry->entryID}{@SID_ARG_2ND}">{$entry->subject}</a></h3>
							...
							<div class="contentHeader">
								...
								{if USER_BLOG_ENABLE_RATING}<p class="rating light smallFont">{lang}wcf.user.blog.entry.rating{/lang} <span id="com.woltlab.wcf.user.blog.entry-ratingOutput{@$entry->entryID}">{@$entry->getRatingOutput()}</span></p>{/if}
								
								<p class="light firstPost">{lang}wcf.user.blog.entry.by{/lang} <a href="index.php?page=User&amp;userID={@$user->userID}{@SID_ARG_2ND}" title="{lang username=$user->username}wcf.user.viewProfile{/lang}">{$username}</a>, {@$entry->time|time}</p>
							</div>
